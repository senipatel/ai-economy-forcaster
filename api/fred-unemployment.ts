...
export default async function handler(req: any, res: any) {
  ...
  try {
    const url = `https://api.stlouisfed.org/fred/series/observations?series_id=UNRATE&api_key=${API_KEY}&file_type=json&sort_order=asc`;
    ...
    const formatted = data.observations
      ...
      .map((o: any) => ({
        ...
        unemployment: Number(o.value),
      }));

    res.status(200).json(formatted);
  } catch (err: any) {
    console.error("Unemployment proxy error:", err);
