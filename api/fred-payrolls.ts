...
export default async function handler(req: any, res: any) {
  ...
  try {
    const url = `https://api.stlouisfed.org/fred/series/observations?series_id=PAYEMS&api_key=${API_KEY}&file_type=json&sort_order=asc`;
    ...
    const formatted = data.observations
      ...
      .map((o: any) => ({
        ...
        payrolls: Number(o.value), // Already in thousands
      }));

    res.status(200).json(formatted);
  } catch (err: any) {
    console.error("Payrolls proxy error:", err);
