...
export const InflationChart = ({ onDataChange }: { onDataChange?: (data: any[]) => void }) => {
  ...
  const handleDownloadImage = async () => {
    ...
    try {
      ...
      link.download = `inflation-${chartType}-chart.png`;
  ...
  const handleDownloadCSV = () => {
    try {
      const headers = ["Date", "Inflation Rate (%)"];
      const csvRows = [headers.join(",")];

      displayData.forEach((row) => {
        const date = String(row.date || "");
        const inflation = String(row.inflation || "");
        csvRows.push([date, inflation].join(","));
      });
      ...
      link.setAttribute("download", `inflation-data-${range}.csv`);
