...
export const FedFundsChart = ({ onDataChange }: { onDataChange?: (data: any[]) => void }) => {
  ...
  // ================================================================
  // 1. Fetch from /api/fred-fedfunds
  // ================================================================
  useEffect(() => {
    ...
    fetch("/api/fred-fedfunds")
      ...
      .catch((err) => {
        console.error("Fed Funds proxy failed → using placeholder:", err);
        toast({
          title: "Using demo data",
          description: "Real Fed Funds Rate unavailable",
          variant: "destructive",
        });
  ...
  const handleDownloadImage = async () => {
    ...
    try {
      ...
      link.download = `fedfunds-${chartType}-chart.png`;
  ...
  const handleDownloadCSV = () => {
    try {
      const headers = ["Date", "Federal Funds Rate (%)"];
      const csvRows = [headers.join(",")];

      displayData.forEach((row) => {
        const date = String(row.date || "");
        const rate = String(row.rate || "");
        csvRows.push([date, rate].join(","));
      });
      ...
      link.setAttribute("download", `fedfunds-data-${range}.csv`);
  ...
  const interval = Math.max(0, invalidateInterval(displayData.length));
...
// Helper to avoid too-dense ticks
function invalidateInterval(length: number): number {
