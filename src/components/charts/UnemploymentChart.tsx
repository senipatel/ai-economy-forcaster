// src/components/charts/UnemploymentChart.tsx
...
export const UnemploymentChart = ({ onDataChange }: { onDataChange?: (data: any[]) => void }) => {
  ...
  // ================================================================
  // 1. Fetch from /api/fred-unemployment
  // ================================================================
  useEffect(() => {
    ...
    fetch("/api/fred-unemployment")
      ...
      .catch((err) => {
      console.error("Unemployment proxy failed → using placeholder:", err);
        toast({
          title: "Using demo data",
          description: "Real unemployment data unavailable",
          variant: "destructive",
        });
    ...
    function fallbackToPlaceholder() {
      ...
      const placeholder = labels.map((lab, idx) => ({
        date: lab,
        unemployment: +(4 + Math.abs(Math.sin(idx / 9)) + Math.random() * 0.8).toFixed(2),
      }));
  ...
  // ================================================================
  // 2. Update display + calculate average
  // ================================================================
  useEffect(() => {
  ...
  const handleDownloadImage = async () => {
    const chartElement = document.getElementById("unemployment-chart");
    if (!chartElement) return;

    try {
      const canvas = await html2canvas(chartElement, { backgroundColor: null, scale: 2 });
      const link = document.createElement("a");
      link.download = `unemployment-${chartType}-chart.png`;
  ...
  const handleDownloadCSV = () => {
    try {
      const headers = ["Date", "Unemployment Rate (%)"];
      const csvRows = [headers.join(",")];

      displayData.forEach((row) => {
        const date = String(row.date || "");
        const unemployment = String(row.unemployment || "");
        csvRows.push([date, unemployment].join(","));
      });
      ...
      link.setAttribute("download", `unemployment-data-${range}.csv`);
  ...
  const average = displayData.reduce((s, d) => s + (d.unemployment || 0), 0) / (displayData.length || 1);
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Chart */}
      <ResponsiveContainer width="100%" height={400} id="unemployment-chart">
        {chartType === "line" ? (
          ...
          <YAxis stroke="hsl(var(--foreground))">
            <Label value="Unemployment Rate (%)" angle={-90} position="center" dx={-30} />
          </YAxis>
          <Tooltip
            contentStyle={{
