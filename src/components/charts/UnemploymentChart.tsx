...
export const UnemploymentChart = () => {
  ...
  useEffect(() => {
    ...
    const placeholderData = labels.map((lab, idx) => ({ date: lab, unemployment: +((4 + Math.abs(Math.sin(idx / 9)) + Math.random() * 0.8).toFixed(2)) }));
    setDataAll(placeholderData);
    setCachedData(cacheKey, placeholderData);
    setLoading(false);
  }, []);

  useEffect(() => { if (!dataAll.length) return; const months = RANGE_MAP[range]; setDisplayData(dataAll.slice(-months)); }, [dataAll, range]);
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.download = "unemployment-chart.png";
    ...
    <div className="space-y-4" >
      ...
      <ResponsiveContainer width="100%" height={400} id="unemployment-chart">
        <LineChart data={displayData} margin={{ top: 20, right: 20, bottom: 80, left: 20 }}>
          ...
          <ReferenceLine y={average} stroke="hsl(var(--muted-foreground))" strokeDasharray="5 5" label={`Avg: ${average.toFixed(1)}%`} />
          <Legend verticalAlign="top" align="center" />
          <Line type="monotone" dataKey="unemployment" stroke="hsl(var(--chart-3))" strokeWidth={3} name="Unemployment Rate (%)" dot={showDots ? { fill: "hsl(var(--chart-3))", r: 4 } : false} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
