// src/components/charts/PayrollsChart.tsx
...
export const PayrollsChart = ({ onDataChange }: { onDataChange?: (data: any[]) => void }) => {
  ...
  useEffect(() => {
    ...
    fetch("/api/fred-payrolls")
      ...
      .catch((err) => {
        console.error("Payrolls proxy failed → using placeholder:", err);
        toast({
          title: "Using demo data",
          description: "Real payrolls data unavailable",
          variant: "destructive",
        });
    ...
    function fallbackToPlaceholder() {
      ...
      const placeholder = labels.map((lab, idx) => ({
        date: lab,
        payrolls: Math.round(150 + Math.sin(idx / 6) * 12 + Math.random() * 8),
      }));
