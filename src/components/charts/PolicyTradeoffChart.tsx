...
export const PolicyTradeoffChart = () => {
  ...
  useEffect(() => {
    ...
    const placeholderData = labels.map((lab, idx) => ({ date: lab, growth: +((Math.sin(idx / 10) * 1.2 + Math.random() * 0.8).toFixed(2)), inflation: +((2 + Math.cos(idx / 8) * 0.9 + Math.random() * 0.5).toFixed(2)) }));
      ...
      <ResponsiveContainer width="100%" height={400} id="policy-tradeoff-chart">
        <LineChart data={displayData} margin={{ top: 20, right: 20, bottom: 80, left: 20 }}>
          ...
          <YAxis stroke="hsl(var(--foreground))">
            <Label value="Value" angle={-90} position="center" dx={-30} />
          </YAxis>
          <Tooltip contentStyle={{ backgroundColor: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8 }} />
          <Legend verticalAlign="top" align="center" />
          <Line type="monotone" dataKey="growth" stroke="hsl(var(--chart-1))" strokeWidth={3} name="Growth (%)" dot={showDots ? { fill: "hsl(var(--chart-1))", r: 4 } : false} />
          <Line type="monotone" dataKey="inflation" stroke="hsl(var(--chart-2))" strokeWidth={2} name="Inflation (%)" dot={showDots ? { fill: "hsl(var(--chart-2))", r: 3 } : false} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
