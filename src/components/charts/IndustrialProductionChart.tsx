...
export const IndustrialProductionChart = ({ onDataChange }: { onDataChange?: (data: any[]) => void }) => {
  ...
  const handleDownloadImage = async () => {
    ...
    try {
      ...
      link.download = `industrial-production-${chartType}-chart.png`;
  ...
  const handleDownloadCSV = () => {
    try {
      const headers = ["Date", "Industrial Production"];
      const csvRows = [headers.join(",")];

      displayData.forEach((row) => {
        const date = String(row.date || "");
        const ip = String(row.ip || "");
        csvRows.push([date, ip].join(","));
      });
      ...
      link.setAttribute("download", `industrial-production-data-${range}.csv`);
