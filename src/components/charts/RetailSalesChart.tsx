...
export const RetailSalesChart = () => {
  ...
  useEffect(() => {
    ...
    const placeholderData = labels.map((lab, idx) => ({ date: lab, retail: +((100 + Math.sin(idx / 7) * 10 + Math.random() * 5).toFixed(2)) }));
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.download = "retail-chart.png";
      ...
      <ResponsiveContainer width="100%" height={400} id="retail-chart">
        <LineChart data={displayData} margin={{ top: 20, right: 20, bottom: 80, left: 20 }}>
          ...
          <YAxis stroke="hsl(var(--foreground))">
            <Label value="Retail Sales" angle={-90} position="center" dx={-30} />
          </YAxis>
          <Tooltip contentStyle={{ backgroundColor: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: 8 }} />
          <Legend verticalAlign="top" align="center" />
          <Line type="monotone" dataKey="retail" stroke="hsl(var(--chart-4))" strokeWidth={3} name="Retail Sales" dot={showDots ? { fill: "hsl(var(--chart-4))", r: 4 } : false} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
